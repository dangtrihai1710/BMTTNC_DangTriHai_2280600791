...
def railfence_encrypt():  # Sửa tên hàm để tránh xung đột
    data = request.json
    plain_text = data['plain_text']
    key = int(data['key'])
...
def railfence_decrypt():  # Sửa tên hàm để tránh xung đột
    data = request.json
    cipher_text = data['cipher_text']
    key = int(data['key'])
...
playfair_cipher = PlayfairCipher()  # Sửa tên class nếu cần
